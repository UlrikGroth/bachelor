function varargout = solver_sBPDN_W1( A, W, b, epsilon, mu, x0, z0, opts, varargin )
% SOLVER_SBPDN_W1 Weighted BPDN problem. Uses smoothing.
% [ x, out, opts ] = solver_sBPDN_W1( A, W, b, epsilon, mu, x0, z0, opts )
%    Solves the smoothed basis pursuit denoising problem
%        minimize norm(W*x,1) + 0.5*mu*(x-x0).^2
%        s.t.     norm(A*x-b,1) <= epsilon
...
%       then the constraints are     norm(A*x-b,1) <= epsilon   AND  x >= 0
%    If "box" is a field in "opts" and opts.box = [l,u]
%       then the constraints are    norm(A*x-b,1) <= epsilon   AND  l <= x <= u
%
...
if epsilon > 0
    prox        = { prox_linf( epsilon ), proj_linf(proxScale) };
else

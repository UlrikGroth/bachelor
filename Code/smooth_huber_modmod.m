function op = smooth_huber_modmod(tau ,lambda)

%SMOOTH_HUBER_MOD   Huber function generation.
%   FUNC = SMOOTH_HUBER_MOD( TAU ) returns a function handle that implements
%
...
function [ v, g ] = smooth_huber_impl(x)
  ...
  smallSet    = ( abs(x) <= tau );
  v           = lambda*sum(smallSet.*( 0.5*(abs(x).^2)./tau ) + (~smallSet).*( abs(x) - tau/2)) ;
  if nargout > 1
      g = lambda*x./max(tau,abs(x));
  end
end % new method
